...
def load_data_flatten(file_path):
        ...
        if line.strip():  # If the line is not blank
            ...
            if current_block:
                ...
                for i in range(block_length):
                    current_block[i].append(i / block_length)
                data.extend(current_block)  # Flatten the block into individual frames
                labels.extend([digit] * block_length)  # Extend labels
                current_block = []
                block_count += 1
    ...
    if current_block:
        ...
        labels.extend([digit] * block_length)

    # Ensure that each inner list in data is of the same length
        ...
        if line.strip():  # If the line is not blank
            ...
            if current_block:
                for i in range(len(current_block)):
                    current_block[i].append(i/len(current_block))
...
plt.title("Confusion Matrix for E-M GMM Classifier + Time Testing Set")
plt.savefig("confusionmatrixTimeGMM")
plt.show()
